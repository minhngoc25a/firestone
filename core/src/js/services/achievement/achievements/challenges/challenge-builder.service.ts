...
import {
    DeckbuildingNoCardWithLetterInNameReq
} from '../requirements/deckbuilding/deckbuilding-no-card-with-letter-in-name-req';
...
import {
    WinAgsinstClassInRankedStandardInLimitedTimeReq
} from '../requirements/win-against-class-in-ranked-standard-in-limited-time-req';
    ...
    private buildRequirement(rawReq: RawRequirement): Requirement {
        ...
        switch (rawReq.type) {
            case 'DUNGEON_RUN_STEP':
                return DungeonRunStepReq.create(rawReq);
            case 'MONSTER_HUNT_STEP':
                return MonsterHuntStepReq.create(rawReq);
            case 'RUMBLE_RUN_STEP':
                return RumbleRunStepReq.create(rawReq);
            case 'GAME_TYPE':
                return GameTypeReq.create(rawReq);
            case 'RANKED_MIN_LEAGUE':
                return StandardRankedMinLeagueReq.create(rawReq);
            case 'RANKED_FORMAT_TYPE':
                return FormatTypeReq.create(rawReq);
            case 'SCENARIO_IDS':
                return ScenarioIdReq.create(rawReq);
            case 'EXCLUDED_SCENARIO_IDS':
                return ExcludedScenarioIdReq.create(rawReq);
            case 'MULLIGAN_DONE':
                return MulliganDoneReq.create(rawReq);
            case 'GAME_WON':
                return GameWonReq.create(rawReq);
            case 'GAME_TIE':
                return GameTieReq.create(rawReq);
            case 'GAME_MIN_TURNS':
                return GameMinTurnsReq.create(rawReq);
            case 'GAME_MAX_TURN': // deprecated, there for backward-compatibilty
            case 'GAME_MAX_TURNS':
                return GameMaxTurnsReq.create(rawReq);
            case 'PLAYER_HERO':
                return PlayerHeroReq.create(rawReq);
            case 'PLAYER_CLASS':
                return PlayerClassReq.create(rawReq, this.cards);
            case 'CORRECT_OPPONENT':
                return CorrectOpponentReq.create(rawReq);
            case 'CORRECT_STARTING_HEALTH':
                return CorrectStartingHealthReq.create(rawReq);
            case 'SCENE_CHANGED_TO_GAME':
                return SceneChangedToGameReq.create(rawReq);
            case 'CARD_PLAYED_OR_CHANGED_ON_BOARD':
                return CardPlayedOrChangedOnBoardReq.create(rawReq);
            case 'BATTLEGROUNDS_HERO_SELECTED':
                return BattlegroundsHeroSelectedReq.create(rawReq);
            case 'CARD_PLAYED_OR_ON_BOARD_AT_GAME_START':
                return CardPlayedOrOnBoardAtGameStartReq.create(rawReq);
            case 'CARD_NOT_PLAYED':
                return CardNotPlayedReq.create(rawReq);
            case 'CARD_DRAWN_OR_RECEIVED_IN_HAND':
                return CardDrawnOrReceivedInHandReq.create(rawReq);
            case 'MINION_SUMMONED':
                return MinionSummonedReq.create(rawReq);
            case 'SECRET_TRIGGERED':
                return SecretTriggeredReq.create(rawReq);
            ...
            case 'PASSIVE_BUFF':
                return PassiveBuffReq.create(rawReq);
            case 'MINION_ATTACK_ON_BOARD':
                return MinionAttackReq.create(rawReq);
            case 'HEALTH_AT_END':
                return HealthAtEndReq.create(rawReq);
            case 'DAMAGE_AT_END':
                return DamageAtEndReq.create(rawReq);
            case 'FATIGUE_DAMAGE':
                return FatigueDamageReq.create(rawReq);
            case 'ARMOR_AT_END':
                return ArmorAtEndReq.create(rawReq);
            case 'TOTAL_DAMAGE_TAKEN':
                return TotalDamageTakenReq.create(rawReq);
            case 'TOTAL_HERO_HEAL':
                return TotalHeroHealReq.create(rawReq);
            case 'TOTAL_DISCARD':
                return TotalDiscardedCardsReq.create(rawReq);
            case 'TOTAL_DAMAGE_DEALT':
                return TotalDamageDealtReq.create(rawReq);
            case 'TOTAL_ARMOR_GAINED':
                return TotalArmorGainReq.create(rawReq);
            case 'MINIONS_CONTROLLED_DURING_TURN':
                return MinionsControlledDuringTurnReq.create(rawReq);
            case 'WIN_STREAK_LENGTH':
                return WinStreakReq.create(rawReq);
            case 'TOTAL_CARDS_PLAYED':
                return TotalCardsPlayedReq.create(rawReq, this.cards);
            case 'TOTAL_MINIONS_SUMMONED':
                return TotalMinionsSummonedReq.create(rawReq, this.cards);
            case 'SAME_MINION_ATTACK_TIMES':
                return SameMinionAttackReq.create(rawReq, this.cards);

            // The very specific reqs
            case 'LAST_DAMAGE_DONE_BY_MINION':
                return LastDamageDoneByMinionReq.create(rawReq);
            case 'BOARD_FULL_OF_SAME_LEGENDARY_MINION':
                return BoardFullOfSameLegendaryMinionReq.create(rawReq, this.cards);
            case 'WINS_AGAINST_CLASS_IN_RANKED_STANDARD_IN_LIMITED_TIME':
                return WinAgsinstClassInRankedStandardInLimitedTimeReq.create(rawReq);
            case 'RESUMMONED_RECURRING_VILLAIN':
                return ResummonRecurringVillainRew.create(rawReq);
            case 'CARDS_WITH_SAME_ATTRIBUTE_PLAYED':
                return CardWithSameAttributePlayedReq.create(rawReq, this.cards);

            // The deckbuilding reqs
            case 'DECK_CLASSIC':
                return DeckbuildingClassicReq.create(rawReq, this.cards);
            case 'DECK_RARITY':
                return DeckbuildingRarityReq.create(rawReq, this.cards);
            case 'DECK_MECHANIC':
                return DeckbuildingMechanicReq.create(rawReq, this.cards);
            case 'DECK_TYPE':
                return DeckbuildingTypeReq.create(rawReq, this.cards);
            case 'DECK_CARD_ATTRIBUTE_VALUE':
                return DeckbuildingCardAttributeReq.create(rawReq, this.cards);
            case 'DECK_CARD_TEXT_VALUE':
                return DeckbuildingTextReq.create(rawReq, this.cards);
            case 'DECK_CARD_TEXT_NUMBER_OF_WORDS':
                return DeckbuildingTextNumberOfWordsReq.create(rawReq, this.cards);
            case 'DECK_NO_CARD_WITH_LETTER_IN_NAME':
                return DeckbuildingNoCardWithLetterInNameReq.create(rawReq, this.cards);
            case 'DECK_CARD_NAME':
                return DeckbuildingCardNameReq.create(rawReq, this.cards);
            case 'DECK_CARD_COST':
                return DeckbuildingCostReq.create(rawReq, this.cards);
            ...
            case 'GLOBAL_STAT':
                return GlobalStatReq.create(rawReq);

            case 'BATTLEGROUNDS_FINISH':
                return BattlegroundsFinishReq.create(rawReq);
            case 'BATTLEGROUNDS_RANK':
                return BattlegroundsRankReq.create(rawReq, this.memoryInspection);
            case 'BATTLEGROUNDS_TRIPLE_PLAY':
                return BattlegroundsTriplePlayReq.create(rawReq);
